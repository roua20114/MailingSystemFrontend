...
// ── Traductions des valeurs API → libellés français ──────────────────────────
...
export function DirectorDispatchView({ mail, open, onClose }: Props) {
  ...
  // Priorité actuelle traduite (pour le SelectItem "Conserver")
  const priorityActuelleFr = PRIORITY_FR[mail.priority] ?? mail.priority;
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border p-4 space-y-2.5">
              ...
              {[
                ...
                ['Type',         TYPE_FR[mail.type] ?? mail.type],        // ← traduit
                ...
                <div>
                  ...
                  <p className="text-xs text-primary/80 mt-0.5">
                    ...
                    {mail.aiConfidenceScore != null &&
                      ` (${Math.round(mail.aiConfidenceScore * 100)} % de confiance)`}
                  </p>
                  {mail.aiSummary && (
            ...
            {mail.assignedTo && (
              <div className="rounded-lg border bg-success/5 border-success/20 p-3 text-xs space-y-1">
                ...
                {mail.assignedDepartment && (
                  <p>Département : <strong>{mail.assignedDepartment.name}</strong></p>
                )}
                {mail.instructions && (
                  <p className="text-muted-foreground mt-1">{mail.instructions}</p>
                )}
              </div>
            )}
            ...
            {canAssign && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Select
                    value={assignedDept}
                    onValueChange={v => setAssignedDept(v === 'none' ? '' : v)}
                  >
                    <SelectTrigger className="mt-1 h-9">
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      {/* Valeur actuelle traduite — la valeur API reste inchangée */}
                      <SelectItem value="keep">
                        Conserver ({priorityActuelleFr})
                      </SelectItem>
